...
describe('sync transports', () => {
	it('filters self-originated broadcast channel messages', () => {
		const transport = createBroadcastChannelTransport('stately-sync', {
			origin: 'local-origin',
	...
	it('filters self-originated storage events and writes serialized payloads', () => {
		...
		const transport = createStorageEventTransport('stately-sync', {
			origin: 'local-origin',
			window: {
		...
		for (const listener of listeners) {
			listener({ key: 'stately-sync', newValue: JSON.stringify({ origin: 'local-origin', state: { count: 2 } }) });
			listener({ key: 'stately-sync', newValue: JSON.stringify({ origin: 'remote-origin', state: { count: 3 } }) });
		}
